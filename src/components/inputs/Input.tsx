      ...
      >
        ...
        >
          ...
          <input
            id={id}
            ref={ref}
            className={`peer block w-full appearance-none rounded-xl border-2 bg-transparent p-3 text-sm transition duration-300 focus:outline-none focus:ring-2 
          ${
            error
              ? `${cn(
          ...
          <label
            htmlFor={id}
            className={`${cn(
              `absolute left-1 top-2 z-10 origin-[0] -translate-y-4 scale-75 transform gap-2 bg-white px-2 text-sm duration-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:scale-100 peer-focus:top-2 peer-focus:-translate-y-5 peer-focus:scale-75 peer-focus:px-2`,
              placeholderClass,
            )} 
            ${
              error
          ...
          >
            <div className="flex items-center justify-start gap-2">
              <AnimatePresence initial={false}>
                ...
                <motion.div
                  key={`placeholder_text_${id}`}
                  layoutId={`placeholder_text_${id}`}
                >
                  {placeholder}
                  {required && (
                    <span>
